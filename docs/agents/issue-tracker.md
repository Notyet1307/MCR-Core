...
- **Blocking**: use GitHub's native issue dependencies. Where dependencies aren't available, fall back to a `Blocked by: #<n>` line.
- **Frontier query**: choose the first open, unassigned child without an open blocker.
- **Claim**: `gh issue edit <n> --add-assignee @me`.
- **Resolve**: comment with the answer, close the child, then add its conclusion to the map's Decisions-so-far.
